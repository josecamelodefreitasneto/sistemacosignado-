/* front-constructor */
import React from 'react';
import ArquivoCampos from '../../cruds/arquivo/ArquivoCampos';
import FormEdit from '../../../fc/components/FormEdit';
import FormItemInput from '../../../antd/form/FormItemInput';
import LayoutApp from '../../../fc/components/LayoutApp';
import UString from '../../misc/utils/UString';
import {Card} from 'antd';
import {Col} from 'antd';
import {Row} from 'antd';
import {Tabs} from 'antd';
const TabPane = Tabs.TabPane;

export default class ArquivoEditAbstract extends FormEdit {
	init2() {
		this.init3();
	}
	constructor(props) {
		super(props);
		this.init("Arquivo", ArquivoCampos.getInstance());
	}
	getTabs() {
		return (
			<Tabs
			tabPosition={"top"}
			activeKey={this.state.abaSelecionada}
			defaultActiveKey={"Geral"}
			onChange={s => this.setAbaSelecionada(s)}>
				<TabPane key={"Geral"} tab={"Geral"}>
					{this.abaGeral()}
				</TabPane>
			</Tabs>
		);
	}
	abaGeral() {
		if (!UString.equals(this.state.abaSelecionada, "Geral")) {
			return null;
		}
		const cps = this.getCampos();
		return (
			<Row gutter={24}>
				{this.grupo_geral_0(cps)}
			</Row>
		);
	}
	inputNome() {
		return <FormItemInput bind={this.getCampos().nome} lg={6}/>;
	}
	inputUri() {
		return <FormItemInput bind={this.getCampos().uri} lg={6}/>;
	}
	inputType() {
		return <FormItemInput bind={this.getCampos().type} lg={6}/>;
	}
	inputTamanho() {
		return <FormItemInput bind={this.getCampos().tamanho} lg={6}/>;
	}
	grupo_geral_0(cps) {
		if (!cps.nome.isVisible() && !cps.uri.isVisible() && !cps.type.isVisible() && !cps.tamanho.isVisible()) {
			return null;
		}
		return (
			<Col lg={24} md={24} sm={24}>
				<Card size={"small"} style={LayoutApp.EMPTY.get()}>
					<Row gutter={24}>
						{this.inputNome()}
						{this.inputUri()}
						{this.inputType()}
						{this.inputTamanho()}
					</Row>
				</Card>
			</Col>
		);
	}
	getTitleImpl() {
		return "Arquivo";
	}
	getCampos() {
		return super.getCampos();
	}
	isAuditada() {
		return false;
	}
	init3() {}
	setWidthForm = o => this.setState({widthForm:o});
	setAbaSelecionada = o => this.setState({abaSelecionada:o});
}

ArquivoEditAbstract.defaultProps = FormEdit.defaultProps;
