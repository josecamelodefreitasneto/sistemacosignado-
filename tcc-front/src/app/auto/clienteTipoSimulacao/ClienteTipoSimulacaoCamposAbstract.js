...
export default class ClienteTipoSimulacaoCamposAbstract extends EntityCampos {
	...
	initImpl() {
		this.nome = this.newString("Nome", 24, true, "Geral");
		this.init2();
	...
	setCampos(o) {
		...
		this.original = o;
		this.to = ClienteTipoSimulacaoUtils.getInstance().clonar(o);
		this.id.clear();
	...
	setJson(obj) {
		this.checkInstance();
		let json = obj;
		let o = ClienteTipoSimulacaoUtils.getInstance().fromJson(json);
		this.setCampos(o);
		let itensGrid = ClienteTipoSimulacaoConsulta.getInstance().getDataSource();
		if (UCommons.notEmpty(itensGrid)) {
			let itemGrid = itensGrid.byId(o.getId());
			if (UCommons.notEmpty(itemGrid)) {
				ClienteTipoSimulacaoUtils.getInstance().merge(o, itemGrid);
			}
		}
		return o;
	}
	...
	houveMudancas() {
		...
		return !ClienteTipoSimulacaoUtils.getInstance().equals(this.original, this.getTo());
	}
	camposAlterados() {
		return ClienteTipoSimulacaoUtils.getInstance().camposAlterados(this.original, this.getTo());
	}
	cancelarAlteracoes() {
	...
	init2() {}
	checkInstance() {
		Sessao.checkInstance("ClienteTipoSimulacaoCampos", this);
	}
}
