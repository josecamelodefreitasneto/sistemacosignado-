...
public abstract class ServiceModelo<T extends EntityModelo> {
	...
	protected void beforeUpdateAfterValidate(T o) {}
...
//	}
	...
	protected ResultadoConsulta consultaBase(MapSO params, FTT<MapSO, T> func) {
		...
		Lst<Integer> ignorar = UList.map(params.getSubList("ignorar"), o -> o.id());
		String busca = UString.toCampoBusca(params.getString("busca"));
		return consultaBase(pagina, ignorar, busca, params, func);
	}
	
	protected ResultadoConsulta consultaBase(Integer pagina, List<Integer> ignorar, String busca, MapSO params, FTT<MapSO, T> func) {
		throw new RuntimeException("???");
	}
