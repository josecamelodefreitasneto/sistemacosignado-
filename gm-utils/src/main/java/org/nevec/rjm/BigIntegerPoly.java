...
public class BigIntegerPoly implements Cloneable {
	 ...
	 * 
	 * @return The widened representation as a polynomial with rational
	 *         coefficients.
	 * @since 2012-03-02
	 * @author Richard J. Mathar
	 */
	 ...
	 *              coefficient, the intermediate coefficients are set to zero.
	 ...
	 * 
	 * @return the number of polynomial coefficients. Differs from the polynomial
	 *         degree by one.
	 * @author Richard J. Mathar
	 */
	public int size() {
	...
	/**
	 * Multiply by another polynomial. Equivalent to a convolution of the polynomial
	 * coefficients.
	 * 
	 ...
	 *         remainder polynomial. This = returnvalue[0] + returnvalue[1]/val .
	 * @since 2012-03-01
	 * @author Richard J. Mathar
	 */
	public BigIntegerPoly[] divideAndRemainder(final BigIntegerPoly val) {
		...
		if (thisSimpl.degree() < valSimpl.degree()) {
			 ...
			 * val. At this point an exception is thrown if the polynomial division cannot
			 * be done with integer coefficients.
			 */
			ret[0] = new BigIntegerPoly();
	 ...
	 * 
	 * @return A (potentially empty) vector of factors, without multiplicity. Only
	 *         factors with non-zero absolute coefficient are generated. This means
	...
	protected Vector<BigIntegerPoly> i2roots() {
		...
		/*
		 * Generate the floating point values of roots. To have some reasonable accuracy
		 * in the results, add zeros to the integer coefficients, scaled by the expected
		 * division with values of b (which are all <= a.firstele). Number of decimal
		 * digits in bsco by using a log2->log10 rough estimate and adding 6 safety
		 * digits
		 */
		...
		/*
		 * for each of the roots z try to see whether c*z^2+a*z+b=0 with integer a, b
		 * and c where b is restricted to a signed divisor of the constant coefficient.
		...
		for (BigComplex z : roo) {
			for (BigInteger bco : b)
				for (BigInteger cco : c) {
					/*
					 * the major reason to avoid the case b=0 is that this would require precaution
					 * of double counting below. Note that this case is already covered by using
					 * iroots().
					 */
					if (bco.signum() != 0) {
