...
public class BigSurdVec implements Comparable<BigSurdVec> {
	...
	/**
	 * ctor given a periodic continued fraction. The value to be represented has a
	 * cont. fraction [a0,a1,a2,...,a0,a1,a2,...]
	 * 
	 * @param a The coefficients a[0], a[1],.. of the period of the continued
	 *          fraction.
	 * @since 2012-03-08
	 * @author Richard J. Mathar
	 */
	public BigSurdVec(Vector<BigInteger> a) throws NumberFormatException {
		/*
		 * this = a0+1/(a1+1/(a2+....) recursively unbundled by setting this=x in a
		 * rational polynomial
		 */
		if (a.size() == 0)
		...
		/*
		 * reached the bottom of the recursion. Solve numer(x)/denom(x) = x to obtain x
		 * as the solution of a quadratic equation. Need to solve numer.a0+number.a1*x =
		 * x*(denom.a0+denom.a1*x) which is numer.a0+number.a1*x =
		...
		case 1:
			/*
			 * if the term in front of the x^2 is zero, defeq=0 results in x=
			 * -defeq.at(0)/defeq.at(1)
			 */
	...
	/**
	 * Combine terms that can be written as a single surd. This unites for example
	 * the terms sqrt(90) and sqrt(10) to 4*sqrt(10).
	 * 
	 * @since 2012-02-15
	 * @author Richard J. Mathar
	 */
	protected void normalize() {
		...
		for (int j = 1; j < terms.size(); j++) {
			...
			for (int ex = 0; ex < newter.size(); ex++) {
				...
				/*
				 * try to merge terms[j] and newter[ex]. todo = r * v with r a rational number
				 * is needed. Replaces v with v+todo = v*(1+r) if this reduction works.
				 */
	...
	/**
	 * Compare algebraic value with oth. Returns -1, 0 or +1 depending on whether
	 * this is smaller, equal to or larger than oth.
	 * 
	...
	public int signum() {
		...
		/*
		 * if all terms have a common sign: take that one offsig is the index of the
		 * first "offending" term in the sense that its sign doese not agree with the
		 * term[0].
		 */
		...
		/*
		 * if there are two terms (now known to have different sign): forward to the
		 * comparison of the two elements as BigSurds
		 */
		if (terms.size() == 2)
			return terms.elementAt(0).compareTo(terms.elementAt(1).negate());

		/*
		 * if there are three terms, move the one with the offending sign to the other
		 * side and square both sides (which looses the sign) to remove all but one
		 * surds. The difference of the squared sides contains at most two terms, which
		 * reduces to the case above. t(0)+t(offbar) <> -t(offs)
		 */
		if (terms.size() == 3) {
			BigSurdVec lhs;
			if (offsig == 2)
...
//			BigSurd rhs = new BigSurd(terms.elementAt(offsig).sqr(), Rational.ONE);
		...
		/*
		 * for a larger number of terms: take a floating point representation with a
		 * small but correct number of digits, and resume with the sign of that one.
		 */
	...
	public BigDecimal BigDecimalValue(MathContext mc) {
		...
		/*
		 * To reduce cancellation errors, loop over increasing local precision until we
		 * are stable to the required result. Keep the old (less precise) estimate in
		 * res[0], and the newer, more precise in res[1].
		 */
		...
		for (int addpr = 1;; addpr += 3) {
			...
			/*
			 * start with an explicitly set first value to avoid treating the initial zero
			 * as being a rounded zero
			 */
	...
	public BigSurdVec negate() {
		...
		/*
		 * no normalization step here, because the negation of all terms does not
		 * introduce new common factors
		 */
		return resul;
	} /* negate */
	...
	public BigSurdVec sqr() {
		/*
		 * Binomial expansion. First the sum of the terms squared, then 2 times the
		 * mixed products.
		 */
		BigSurdVec resul = new BigSurdVec();
	...
	/**
	 * Return a string in the format (number/denom)*()^(1/2). If the discriminant
	 * equals 1, print just the prefactor.
	 * 
