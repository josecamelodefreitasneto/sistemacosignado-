...
class Permutation implements Cloneable {
	...
	/**
	 * The order of this permutation. A non-positive value means that this is not
	 * yet known, because the computation is delayed.
	 */
	private int ord;

	/**
	 * Defines whether the permutation is represented with indices 0..n-1 or with
	 * 1..n The value is either 1 (more complicated) or 0 (simpler to implement).
	 */
	...
	/**
	 * Ctor for the unit permutation. This is either (1,2,..,n) or (0,1,...,n-1)
	 * depending on the internally fixed offset.
	 * 
	...
	/**
	 * ctor starting with a cycle-length specification. If v[0]+v[1]+...+v[] = n is
	 * a partition of n, then we construct a permutation with cycles of length v[0],
	 * v[1], ... v[].
	 * 
	...
	Permutation(final int[][] cycs) {
		...
		for (int c = 0; c < cycs.length; c++) {
			...
			for (int i = 0; i < cl; i++) {
				/*
				 * element cycs[c][i] is permuted to cycs[c][i+1] for i>=0 but the last in the
				 * cycle to cycs[c][0], back to the front.
				 */
	 ...
	 * 
	 * @return a permutation that yields the unit element after multiplication with
	 *         this.
	 */
	...
	/**
	 * Multiply with another permutation (which is placed to the right). The result
	 * is the product of the permutations in the sense that this is acting first,
	 * then 'right' later.
	 * 
	...
	Permutation multiply(final Permutation right) {
		if (images.length != right.images.length)
			throw new IllegalArgumentException(
					"Different permutation lengths " + images.length + " vs " + right.images.length);
		...
		for (int i = 0; i < images.length; i++)
			/*
			 * pay attention to 1-offset convention of permutations here Standard slower
			 * implementation: prod[i] = right.imageOf(imageOf(i+REPOFFSET)); Inlined
			 * version follows:
			 */
	 ...
	 * 
	 * @return True if this is the permutation (1,2,3,...,n) or (0,1,2,3,..n-1),
	 *         depending on the REPOFFSET.
	 */
	...
	/**
	 * Least common multiple of two positive integers. Computed via Euclid's
	 * algorithm.
	 * 
	 * @param a The first positive integer.
	 * @param b The second positive integer.
	...
	/**
	 * Calculate the order. This is computed as the lcm of the lengths of the
	 * cycles.
	 * 
	 * @return the order of the permutation. This is the smallest positive power 0
	 *         of the permutation that yields the unit element.
	 */
	int orderByCycles() {
		/*
	...
	/**
	 * Calculate the order. This is computed by the lcm of the lengths of the
	 * cycles.
	 * 
	 * @param cyc The cycles. Complete in the sense that each moved element in the
	 *            permutation appears exactly once in the array.
	 * @return The order of the permutation. This is the smallest positive power 0
	 *         of the permutation that yields the unit element.
	 */
	 ...
	 * 
	 * @return [0][..] are the integers in the first cycle, [1][...] in the second,
	 *         and so on. Fixed elements of cycle length 1 are not included in the
	 *         result.
	 */
	public int[][] cycles() {
		...
		/*
		 * flag for each integer in the permutation that is has not yet appeared in a
		 * cycle. Default initialiation for booleans is false, so we may skip the
		 * initialization here.
		 */
		...
		while (true) {
			 ...
			 * that loop exited with notmoved=images.length and we terminate the loop that
			 * lists cycles.
			 */
			...
			/*
			 * mark all elements in this cycle as treated and accounted for by tracei[].
			 * Note that tracei may be empty if notmoved was a fixed element, but the
			 * associated element needs to be marked as moved anyway.
			 */
			done[notmoved] = true;
			...
			/*
			 * add this cycle to the vector if non-trivial, i.e., if not for a fixed
			 * element.
			 */
			if (tracei.length > 1)
				c.add(tracei);
	...
	/**
	 * Generate a list of lists that are the cycle representation. In the string all
	 * elements are larger than or equal to 1 and less than or equal to n, 1-based.
	 * 
	 * @return The 0th component are the integers in the first cycle, the 1th
	 *         compoment in the second, and so on.
	 */
	public String cyclRep() {
		int[][] allcyc = cycles();
		String rep = new String();
		/*
		 * there is one case ( the unit element) where no cycles are listed because they
		 * are only containing fixed elements. To vaoid that this disappears entirely,
		 * we print an empty () to let it be visible.
		 */
		if (allcyc.length == 0)
			return new String("()");

		/*
		 * loop over all cycles in the order genereated (ie, such that the small
		 * elements appear early.
		 */
	...
	int imageOf(final int e) {
		...
		/*
		 * if the element is outside the range of the permutation, it is assumed to stay
		 * fixed.
		 */
	...
	/**
	 * Generate the trace of an integer until the repeated image has closed its
	 * cycle.
	 * 
	 * @param s The integer to be traced.
	 * @return a vector [s, images[s], images^2[s]...]. Fixed elements are not
	 *         reported at all, so their return vector is empty.
	 */
	int[] orbit(final int s) {
		...
		if (or.size() >= 2) {
			/*
			 * return only something if the order of the element is at least 2; skip fixed
			 * points.
			 */
	...
	/**
	 * Compare with another permutation. The dominant key is the lenghts of the
	 * permutations. The secondary key (if of the same length) is the left-right
	 * image=by-image comparison.
	 * 
	 * @param oth
	 * @return -1, 0 or +1 depending on whether this is smaller than, equal to or
	 *         greater than oth
	 */
	 ...
	 * 
	 * @return The smallest index (0-based) that does not stay fixed. If all
	 *         elements are fixed, the number of elements is returned.
	 * @since 2015-03-13
	 */
	 ...
	 * 
	 * @return The largest index (0-based) that stays fixed. If all elements are
	 *         moved, -1 is returned.
	 * @since 2015-03-13
	 */
	...
	/**
	 * Smallest element that remains fixed under all permutations of a set. This
	 * means all elements of smaller index are moved by at least one of the
	 * permutations.
	 * 
	 * @param pers The set of permutations that may move the element
	 * @return The minimum 0-based or 1-based element that remains fixed. If the
	 *         pers move a dense set of points, this is 1 higher than the point
	 *         length of the longest permutation.
	 * @since 2015-03-15
	 */
	public static int smallestFixed(Vector<Permutation> pers) {
		...
		for (;; el++) {
			...
			for (Permutation p : pers) {
				/*
				 * does p move el? The function imageOf() fixes all elements that are beyond the
				 * range of the permutation to run this smoothly.
				 */
				if (p.imageOf(el) != el) {
					isfix = false;
					break;
	 ...
	 * 
	 * @param per The list of permutations. These should be the full list of the
	 *            members of a permutation group.
	 * @return The smallest number of points that are not fixed by any non-identity
	 *         permutation.
	 */
	public static int nrMoved(final Permutation[] per) {
		/*
		 * start with the most pessimistic upper bound that all elements are moved. do
		 * not use per[0].nrMoved() here because per[0] may be the identity.
		 */
		int minp = 0;
	 ...
	 * 
	 * @return true if the moved elements have all higher indices than the fixed
	 *         elements.
	 */
	boolean isSorted() {
		if (orderByCycles() == 1)
		...
		/*
		 * there are three cases: all elements moved where m=0, f=-1, all elements
		 * fixed, where m=images.length, f=images.length-1 and the general case.
	...
	boolean isCycSorted() {
		...
		if (cyc.length <= 1)
			...
			/*
			 * compare all pairs cyc[i] and cyc[j], j>i, to test if all elements of cyc[j]
			 * are larger than all elements of cyc[i]
			 */
			for (int i = 0; i < cyc.length - 1; i++)
				for (int j = i + 1; j < cyc.length; j++) {
	...
	/**
	 * Test if the permutation is a pivotal representative of permutations with the
	 * same structure
	 */
	boolean isPivot() {
	...
	/**
	 * Print the permutation as a left-right list of integers. This output is always
	 * using a 1-based representation of the vectors.
	 * 
	 * @since 2015-03-07
	 * @author Richard J. Mathar
	 */
	 ...
	 * Test programs.
	 * 
	 * @param args There is only one single command line argument, the non-negative
	 *             number n .
	 * @since 2010-09-13
	 * @author Richard J. Mathar
	 */
	public static void main(String args[]) {
		...
		if (args.length == 1) {
			...
			int maxc[] = { 0, 1, 2, 3, 4, 6, 6, 12, 15, 20, 30, 30, 60, 60, 84, 105, 140, 210, 210, 420, 420, 420, 420,
					840, 840 };
			/*
			 * build a statistics of cycle lengths in the symmetric groups,
			 * http://oeis.org/A057731 1: 1 1 1: 2 1 3 2: 6 1 9 8 6: 24 1 25 20 30 24 20:
			 * 120 1 75 80 180 144 240: 720 1 231 350 840 504 1470 720 0 0 504 0 420: 5040 1
			 * 763 1232 5460 1344 10640 5760 5040 0 4032 0 3360 0 0 2688: 40320 1 2619 5768
			 * 30996 3024 83160 25920 45360 40320 27216 0 30240 0 25920 24192 0 0 0 0 18144:
			 * 362880 1 9495 31040 209160 78624 584640 86400 453600 403200 514080 0 403200 0
			 * 259200 120960 0 0
			 * 
			 */
			for (int n = 1; n < 10; n++) {
				...
				long cyct = 0;
				/*
				 * loop over the possible cycle lengths (orders of elements) up to the maximum
				 * possible for the symmetric group S_n
				 */
				for (int cylen = 1; cylen <= maxc[n]; cylen++) {
					long ct = 0;
					/*
					 * Start with the mutable permutation (0,1,2,3,...,n-1) and loop over all its
					 * permutations.
					 */
