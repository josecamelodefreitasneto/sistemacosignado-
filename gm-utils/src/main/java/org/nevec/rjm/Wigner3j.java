 ...
 * "Universal factorization of ..[J. Phys. A: Math. Gen.37 (2004) 3259]"</a> R.
 * J. Mathar, <a href="http://vixra.org/abs/1202.0093">Symmetries in Wigner 18-j
 * and 21-j Symbols</a>
 * 
 * @since 2011-02-15
...
public class Wigner3j {
	 ...
	 * whether a 3jm, a 6j or a 9j symbol will be computed. The other arguments are
	 * 6 or 9 integer values, which are the physical (half-integer) values multplied
	 * by 2 and augmented by 1. The order of the 6 or 9 values is as reading the
	 * corresponding standard symbol as first row, then second row (and for the 9j
	 * symbol) third row.
	 * 
	...
	public static void main(String args[]) {
		if (args[0].compareTo("6j") == 0) {
			...
			int j1 = UInteger.toInt(args[1]);
			int j2 = UInteger.toInt(args[2]);
			int j3 = UInteger.toInt(args[3]);
			int m1 = UInteger.toInt(args[4]);
			int m2 = UInteger.toInt(args[5]);
			int m3 = UInteger.toInt(args[6]);
			try {
	...
	/**
	 * The Wigner 3jm symbol (j1,j2,j3,m1,m2,m3). All arguments of the function are
	 * the actual parameters multiplied by 2, so they all allow an integer
	 * representation.
	 * 
	 ...
	 * @return The value of the symbol. Zero if any of the triangular inequalities
	 *         is violated or some parameters are out of range.
	 * @since 2011-02-13
	 * @author Richard J. Mathar
	 */
	...
	/**
	 * Wigner 3jn symbol. For the 6j symbol, the input of the 3 lines is "1 2 3 1 5
	 * 6", "4 5 3 4 2 6" "2j1+1 2j2+1 2j3+1 2l1+1 2l2+1 2l3+1"
	 * 
	 ...
	 *           separated
	 * @param t2 The list of the second half of the triads, indexing j, whitespace
	 *           separated
	 * @param j  The list of the integer values of the angular momenta. They are
	 *           actually the doubled j-values plus 1, whitespace separated. Only as
	 *           many as announced by the m1 parameter are used; trailing numbers
	 *           are ignored. A. Bar-Shalom and M. Klapisch, <a href=
	 *           "http://dx.doi.org/10.1016/0010-4655(88)90192-0">NJGRAF...</a>,
	...
	public static BigSurdVec wigner3j(String m1, String t1, String t2, String j) {
		/*
		 * The first number in the line "m" is the number of angular momenta. The rest
		 * of the line is ignored.
		 */
		
		int m;
		
		try (Scanner s = new Scanner(m1)) {
			m = s.nextInt();
			if (m % 3 != 0) {
		...
		/*
		 * Basic sanity checks. All indices in the first two lines address a number in
		 * the third line, and each index occurs exactly twice.
		 */
		if (ji % 3 != 0)
		...
		/*
		 * maintain a 0-based index which shows where the j-value has its first and
		 * second occurrence in the flattened list of triads.
		 */
		...
		/*
		 * Move on from the 2j+1 values of the input to the j-values. Subtract one and
		 * divide through 2.
		 */
	 ...
	 * Wigner-3jm symbol with each sign of m_i occurring once at the corresponding
	 * l-value.
	 * 
	 * @param tvec
	 * @param J        The list of J-values
	 * @param M        The list of M-values associated with the J. This contains
	 *                 null where the parameter has not yet been set by an outer
	 *                 loop.
	...
	static private BigSurdVec wigner3j(final int[] tvec, final Rational[] J, final Rational[] M, final int[] triadidx) {
		...
		/*
		 * First step is to monitor the triangular conditions on the J. If at least one
		 * is violated, the result is zero. Loop over the triads.
		 */
		...
		/*
		 * the index of the preferred member of the triad list. Preference given to
		 * those dangling in triads where alreaday two others are fixed, then to members
		 * where at least one is fixed, then to smallest associated J-values.
		 */
		int freeM = -1;
		int freeMrank = -1;
		for (int i = 0; i < triadidx.length; i++) {
			...
			if (M[triadidx[i]] == null) {
				/*
				 * two cases: value is fixed implicitly because already two others values are
				 * set in the triad. or it is still to maintain its own explicit loop.
				 */
				int triadn = i / 3;
				int triadr = i % 3;
				/*
				 * the neighbors in the triad have indices triadn*3+ (tiradr+1) mod 3 and
				 * triadn*3+(triadr+2) mod3
				 */
		...
		if (freeM >= 0) {
			...
			if (M[triadidx[freeM]] == null) {
				...
				int triadr = freeM % 3;
				/*
				 * the neighbors in the triad have indices triadn*3+ (triadr+1) mod 3 and
				 * triadn*3+(triadr+2) mod3
				 */
				int nei1 = 3 * triadn + (triadr + 1) % 3;
				int nei2 = 3 * triadn + (triadr + 2) % 3;
				if (M[triadidx[nei1]] == null || M[triadidx[nei2]] == null) {
					...
					/*
					 * Set its value and the value at its companion j-value. Sum of the three
					 * m-values in the triad is to be zero for a non-zero contribution.
					 */
					...
					/*
					 * No contribution if the m-value enforced by the other two entries is outside
					 * the range -|J|..|J| enforced by its associated J-value. One could essentially
					 * remove this branching and let wigner3j() decide on this, but this is
					 * inefficient.
					 */
					...
					/*
					 * zero contribution if this m-value cannot be set to any value compatible with
		...
		/*
		 * reached the bottom of the loop where all M-values are assigned. Build the
		 * product over all Wigner-3jm values and the associated sign.
		 */
		res = BigSurdVec.ONE;
		for (int ji = 0; ji < triadidx.length; ji += 3) {
			...
			/*
			 * if a partial product yields zero, the total product is zero, too, and offers
			 * an early exit.
			 */
			if (res.signum() == 0)
		...
		/*
		 * The overal sign is product_{J-Mpairs} (-1)^(J-M). This is an integer because
		 * all the J-M are integer.
		 */
		Rational sig = new Rational();
		for (int ji = 0; ji < J.length; ji++)
			sig = sig.add(J[ji]).subtract(M[ji]);
		/*
		 * sign depends on the sum being even or odd. We assume that "sig" is integer
		 * and look only at the numerator
		 */
	...
	/**
	 * The Wigner 3jm symbol (j1,j2,j3,m1,m2,m3). Warning: there is no check that
	 * each argument is indeed half-integer.
	 * 
	...
	static protected BigSurd wigner3jm(Rational j1, Rational j2, Rational j3, Rational m1, Rational m2, Rational m3) {
		...
		/*
		 * (-)^(j1-j2-m3)*delta(-m3,m1+m2)*sqrt[ (j3+j1-j2)! (j3-j1+j2)! (j1+j2-j3)!
		 * /(j1+j2+j3+1)! *(j3-m)!*(j3+m)!(j1-m1)!*(j1+m1)!*(j2-m2)!*(j2+m2)!] *sum_k
		 * (-1)^k/[k!(j1+j2-j3-k)!(j1-m1-k)!(j2+m2-k)!(j3-j2+m1+k)!)*(j3-j1-m2+k)!]
		 */

		/*
		 * It is tacitly assumed that all the major j_i, m_i values are in the integer
		 * range. This is implicitly plausible since otherwise the execution times of
		 * the following loop over the k-values would be immense.
		 */
