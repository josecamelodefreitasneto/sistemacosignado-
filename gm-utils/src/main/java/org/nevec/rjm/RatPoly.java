...
/**
 * A uni-variant polynomial with rational coefficients. Alternatively to be
 * interpreted as a sequence which has the polynomial as an (approximate)
 * generating function.
 * 
 * @since 2006-06-25
...
class RatPoly {
	/**
	 * The list of all coefficients, ascending exponents. Starting with a0, then a1,
	 * representing a value a0+a1*x+a2*x^2+a3*x^3+...
	 */
	 ...
	 * 
	 * @param L the coefficients a0, a1, a2, a3,.., A deep copy of the these is
	 *          created.
	 * @author Richard J. Mathar
	 */
	...
	public RatPoly(final Vector<BigInteger> A, final Vector<BigInteger> B, int nmax) {
		/*
		 * To allow common initialization with the signature below, the main body is
		 * assembled in a separate function.
		 */
		init(A, B, nmax);
	}
	 ...
	 * Constructor from a hypergeometric series.
	 * 
	 * @param A the list of values in the numerator of AFB. At least one of these
	 *          values must be a negative integer, which implicitly determines the
	 *          order of the new polynomial.
	 * @param B the list of values in the denominator of AFB
	 * @since 2009-08-05
	 * @author Richard J. Mathar
	 */
	 ...
	 * 
	 * @param n the zero-based index of the coefficient. n=0 for the constant term.
	 * If the polynomial has not yet the degree to need this coefficient, the
	 * intermediate coefficients are implicitly set to zero.
	 * 
	 ...
	 *              term. If the polynomial has not yet the degree to need this
	 *              coefficient, the intermediate coefficients are implicitly set to
	 *              zero.
	 * @param value the new value of the coefficient.
	 * @author Richard J. Mathar
	 */
	 ...
	 * @return the product of this with the factor. All coefficients of this have
	 *         been multiplied individually by the factor.
	 * @author Richard J. Mathar
	 */
	...
	public RatPoly pow(final int n) throws ArithmeticException {
		...
		if (n < 0)
			...
			/*
			 * this ought probably be done with some binary representation of the power and
	...
	/**
	 * Raise to a rational power. The result is the taylor expansion of this,
	 * truncated at the first term that remains undetermined based on the current
	...
	public RatPoly pow(final Rational r) throws ArithmeticException {
		...
		/*
		 * scale the polynomial by division through the expansion coefficient of the
		 * absolute term
		 */
	...
	public RatPoly add(final RatPoly val) {
		RatPoly resul = new RatPoly();
		/*
		 * the degree of the result is the larger of the two degrees (before simplify()
		 * at least).
		 */
	...
	public RatPoly divide(final RatPoly val, int nmax) {
		...
		RatPoly resul = new RatPoly();
		/*
		 * todo: If the polynomial division is exact, we could leave the loop earlier,
		 * indeed
		 */
		for (int n = 0; n <= nmax; n++) {
	...
	public RatPoly[] divideAndRemainder(final RatPoly val) {
		...
		if (thisSimpl.degree() < valSimpl.degree()) {
			...
			/*
			 * long division. Highest degree by dividing the highest degree of this thru
			 * val.
			 */
			ret[0] = new RatPoly();
			...
			/*
			 * recurrences: build this - val*(1-termresult) and feed this into another round
			 * of division. Have intermediate ret[0]+ret[1]/val.
			 */
	 ...
	 * 
	 * @return The representation a0,a1,a2,a3,... This is a sort of opposite of the
	 *         ctor that takes a string as an argument.
	 * @since 2008-10-25
	 * @author Richard J. Mathar
	 */
	 ...
	 * 
	 * @return To representation a0+a1*x+a2*x^2+... This does not print the terms
	...
	/**
	 * Simplify the representation. Trailing values with zero coefficients (at high
	 * powers) are deleted. This modifies the polynomial on the stop (does not
	...
	/**
	 * Scale coefficients such that the coefficient in front of the maximum degree
	 * is unity.
	 * 
	 ...
	 * @return the sequence of coefficients is the Mobius transform of the original
	 *         sequence.
	 * @since 2008-12-02
	 * @author Richard J. Mathar
	 */
	 ...
	 * 
	 * @param maxdeg the maximum polynomial degree of the result
	 * @return the sequence of coefficients is the inverse Mobius transform of the
	 *         original sequence.
	 * @since 2008-12-02
	 * @author Richard J. Mathar
	 */
	 ...
	 * 
	 * @param maxdeg the maximum polynomial degree of the result
	 * @return the sequence of coefficients is the inverse binomial transform of the
	 *         original sequence.
	 * @since 2008-10-26
	 * @author Richard J. Mathar
	 */
	 ...
	 * @return The polynomial with all coefficients beyond deg set to zero. If
	 *         newdeg =3, for example the polynomial returned has at most degree 3.
	 *         If newdeg is larger than the degree of this, zeros (at the higher
	 *         orders of x) are appended. That polynomial would have formal degree
	 *         larger than this.
	 * @since 2008-10-26
	 * @author Richard J. Mathar
	 */
	 ...
	 * <a href="http://en.wikipedia.org/wiki/Durand-Kerner_method">Durand Kerner
	 * method</a>
	 * 
	 ...
	 * 
	 * @return The vector of integer roots, with multiplicity. The shows
	 *         alternatingly first a root then its multiplicty, then another root
	 *         and multiplicty etc.
	 * @since 2008-10-26
	 * @author Richard J. Mathar
	 */
	public Vector<BigInteger> iroots() {
		...
		/*
		 * multiply all coefs with the lcm() to get an integer polynomial start with
		 * denominator of first non-zero coefficient.
		 */
		...
		/*
		 * collect the integer roots (multiple roots only once). Since we removed the
		 * zero already above, cand does not contain zeros.
		 */
