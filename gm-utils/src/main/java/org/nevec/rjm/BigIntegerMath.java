...
public class BigIntegerMath {
	...
	public static BigInteger binomial(final int n, final int k) {
		...
		BigInteger n2 = bin;
		for (BigInteger i = BigInteger.valueOf((long) (k - 1)); i.compareTo(BigInteger.ONE) >= 0; i = i
				.subtract(BigInteger.ONE))
			bin = bin.multiply(n2.subtract(i));
		for (BigInteger i = BigInteger.valueOf((long) k); i.compareTo(BigInteger.ONE) == 1; i = i
				.subtract(BigInteger.ONE))
			bin = bin.divide(i);
		return (bin);
	} /* binomial */
	...
	public static BigInteger binomial(final BigInteger n, final BigInteger k) {
		...
		/*
		 * the following version first calculates n(n-1)(n-2)..(n-k+1) in the first
		 * loop, and divides this product through k(k-1)(k-2)....2 in the second loop.
		 * This is rather slow and replaced by a faster version below BigInteger n2 =
		 * bin ; BigInteger i= k.subtract(BigInteger.ONE); for( ;
		 * i.compareTo(BigInteger.ONE) >= 0 ; i = i.subtract(BigInteger.ONE) ) bin =
		 * bin.multiply(n2.subtract(i)); i= BigInteger.valueOf(k); for( ;
		 * i.compareTo(BigInteger.ONE) == 1 ; i = i.subtract(BigInteger.ONE) ) bin =
		 * bin.divide(i);
		 */

		/*
		 * calculate n then n(n-1)/2 then n(n-1)(n-2)(2*3) etc up to
		 * n(n-1)..(n-k+1)/(2*3*..k) This is roughly the best way to keep the individual
		 * intermediate products small and in the integer domain. First replace C(n,k)
		 * by C(n,n-k) if n-k<k.
		 */
		BigInteger truek = k;
	 ...
	 * @author Richard J. Mathar
	 */
	public static BigInteger sigmak(final BigInteger n, final int k) {
	 ...
	 * @author Richard J. Mathar
	 */
	public static Vector<BigInteger> divisors(final BigInteger n) {
	 ...
	 * @author Richard J. Mathar
	 */
	public static BigInteger sigma(final BigInteger n) {
	 ...
	 * @author Richard J. Mathar
	 */
	public static BigInteger isqrt(final BigInteger n) {
		if (n.compareTo(BigInteger.ZERO) < 0)
		...
		while (true) {
			...
			/*
			 * Newton algorithm. This correction is on the low side caused by the integer
			 * divisions. So the value required may end up by one unit too large by the bare
			 * algorithm, and this is caught above by comparing x^2, (x+-1)^2 with n.
			 */
	 ...
	 * 
	 * @param x The non-negative argument. Arguments less than zero throw an
	 *          ArithmeticException.
	 ...
	 * @author Richard J. Mathar
	 */
	public static BigInteger iroot(final BigInteger x, final int n) {
	...
	/**
	 * Evaluate core(n). Returns the smallest positive integer m such that n/m is a
	 * perfect square.
	 * 
	 * @param n The non-negative argument.
	 ...
	 * @author Richard J. Mathar
	 */
	public static BigInteger core(final BigInteger n) {
		if (n.compareTo(BigInteger.ZERO) < 0)
			throw new ArithmeticException("Negative argument " + n);
	 ...
	 * 
	 * @param A The matrix.
	 * @param r The row index of the row to be removed (0-based). An exception is
	 *          thrown if this is outside the range 0 to the upper row index of A.
	 * @param c The column index of the column to be removed (0-based). An exception
	 *          is thrown if this is outside the range 0 to the upper column index
	 *          of A.
	 * @return The depleted matrix. This is not a deep copy but contains references
	 *         to the original.
	 * @since 2010-08-27
	 * @author Richard J. Mathar
	 */
	 ...
	 * @author Richard J. Mathar
	 */
	public static Rational[] solve(final BigInteger[][] A, final BigInteger[] rhs) throws ArithmeticException {
		...
		for (int c = 0; c < cL - 1; c++) {
			...
			for (int r = c + 1; r < rL; r++) {
				/*
				 * skip the cpr=c which actually sets the zero: this element is not visited
				 * again
				 */
	 ...
	 * @author Richard J. Mathar
	 */
	public static BigInteger valueOf(final Vector<BigInteger> c, final BigInteger x) {
		if (c.size() == 0)
	 ...
	 * @since 2009-08-06
	 * @author Richard J. Mathar
	 *         <a href="http://dx.doi.org/10.1080/01630568908816313">P. L. Butzer et
	 *         al, Num. Funct. Anal. Opt. 10 (5)( 1989) 419-488</a>
	 */
