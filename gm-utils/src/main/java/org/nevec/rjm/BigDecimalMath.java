 ...
 * 
 * @since 2009-05-22
 * @author Richard J. Mathar <a href="http://apfloat.org/">apfloat</a>
...
public class BigDecimalMath {
	 ...
	 * http://www.cs.arizona.edu/icon/oddsends/e.htm The precision of the predefined
	 * constant is one less than the string's length, taking into account the
	 * decimal dot. static int E_PRECISION = E.length()-1 ;
	 */
	...
	public static BigDecimal gamma(MathContext mc) {
		...
		if (mc.getPrecision() < GAMMA.precision())
			...
			/*
			 * how many terms: zeta-1 falls as 1/2^(2n+1), so the terms drop faster than
			 * 1/2^(4n+2). Set 1/2^(4kmax+2) < eps. Leading term zeta(3)/(4^1*3) is 0.017.
			 * Leading zeta(3) is 1.2. Log(2) is 0.7
			 */
			int kmax = (int) ((Math.log(eps / 0.7) - 2.) / 4.);
			mcloc = new MathContext(1 + err2prec(1.2, eps / kmax));
			for (int n = 1;; n++) {
				/*
				 * zeta is close to 1. Division of zeta-1 through 4^n*(2n+1) means divion
				 * through roughly 2^(2n+1)
				 */
	...
	public static BigDecimal sqrt(final BigDecimal x, final MathContext mc) {
		...
		for (;;) {
			/*
			 * s = s -(s/2-x/2s); test correction s-x/s for being smaller than the precision
			 * requested. The relative correction is 1-x/s^2, (actually half of this, which
			 * we use for a little bit of additional protection).
			 */
	 ...
	 * 
	 * @param x the non-negative argument.
	 * @return the square root of the BigDecimal rounded to the precision implied by
	 *         x.
	 * @since 2009-06-25
	 * @author Richard J. Mathar
	 */
	 ...
	 * @return The cubic root of the BigDecimal rounded to the precision implied by
	 *         x. The sign of the result is the sign of the argument.
	 * @since 2009-08-16
	 * @author Richard J. Mathar
	 */
	 ...
	 * @param x the non-negative argument.
	 * @return The n-th root of the BigDecimal rounded to the precision implied by
	 *         x, x^(1/n).
	 * @since 2009-07-30
	 * @author Richard J. Mathar
	 */
	public static BigDecimal root(final int n, final BigDecimal x) {
		...
		/*
		 * Specify an internal accuracy within the loop which is slightly larger than
		 * what is demanded by 'eps' below.
		 */
		...
		for (;;) {
			/*
			 * s = s -(s/n-x/n/s^(n-1)) = s-(s-x/s^(n-1))/n; test correction s/n-x/s for
			 * being smaller than the precision requested. The relative correction is
			 * (1-x/s^n)/n,
			 */
	...
	public static BigDecimal hypot(final BigDecimal x, final BigDecimal y) {
		...
		/*
		 * Final rounding. Absolute error in the square root is (y*yerr+x*xerr)/z, where
		 * zerr holds 2*(x*xerr+y*yerr).
		 */
	...
	public static BigDecimal hypot(final int n, final BigDecimal x) {
		...
		/*
		 * Truncate to the precision set by x. Absolute error = in z (square of the
		 * result) is |2*x*xerr|, where the error is 1/2 of the ulp. Two intermediate
		 * protection digits. zerr is a signed value, but used only in conjunction with
		 * err2prec(), so this feature does not harm.
		 */
		...
		/*
		 * Final rounding. Absolute error in the square root is x*xerr/z, where zerr
		 * holds 2*x*xerr.
		 */
	...
	/**
	 * A suggestion for the maximum numter of terms in the Taylor expansion of the
	 * exponential.
	 */
	...
	public static BigDecimal exp(BigDecimal x) {
		...
		if (x.compareTo(BigDecimal.ZERO) < 0) {
			...
			/*
			 * recover the valid number of digits from x.ulp(), if x hits the zero. The
			 * x.precision() is 1 then, and does not provide this information.
			 */
			return scalePrec(BigDecimal.ONE, -(int) (Math.log10(x.ulp().doubleValue())));
		} else {
			/*
			 * Push the number in the Taylor expansion down to a small value where
			 * TAYLOR_NTERM terms will do. If x<1, the n-th term is of the order x^n/n!, and
			 * equal to both the absolute and relative error of the result since the result
			 * is close to 1. The x.ulp() sets the relative and absolute error of the
			 * result, as estimated from the first Taylor term. We want
			 * x^TAYLOR_NTERM/TAYLOR_NTERM! < x.ulp, which is guaranteed if x^TAYLOR_NTERM <
			...
			if (Math.pow(xDbl, TAYLOR_NTERM) < TAYLOR_NTERM * (TAYLOR_NTERM - 1.0) * (TAYLOR_NTERM - 2.0) * xUlpDbl) {
				...
				/*
				 * TAYLOR_NTERM terms to be added means we move x.ulp() to the right for each
				 * power of 10 in TAYLOR_NTERM, so the addition won't add noise beyond what's
				 * already in x.
				 */
				...
				/*
				 * exp(x+deltax) = exp(x)(1+deltax) if deltax is <<1. So the relative error in
				 * the result equals the absolute error in the argument.
				 */
				MathContext mc = new MathContext(err2prec(xUlpDbl / 2.));
				return resul.round(mc);
			} else {
				/*
				 * Compute exp(x) = (exp(0.1*x))^10. Division by 10 does not lead to loss of
				 * accuracy.
				 */
				...
				/*
				 * Final powering by 10 means that the relative error of the result is 10 times
				 * the relative error of the base (First order binomial expansion). This looses
				 * one digit.
				 */
				MathContext mc = new MathContext(expxby10.precision() - exSc);
				/*
				 * Rescaling the powers of 10 is done in chunks of a maximum of 8 to avoid an
				 * invalid operation response by the BigDecimal.pow library or integer overflow.
				 */
				while (exSc > 0) {
	...
	public static BigDecimal exp(final MathContext mc) {
		...
		if (mc.getPrecision() < E.precision())
			...
			/*
			 * Instantiate a 1.0 with the requested pseudo-accuracy and delegate the
			 * computation to the public method above.
			 */
	...
	public static BigDecimal log(BigDecimal x) {
		...
		if (x.compareTo(BigDecimal.ZERO) < 0)
			...
			/*
			 * Map log(x) = log root[r](x)^r = r*log( root[r](x)) with the aim to move
			 * roor[r](x) near to 1.2 (that is, below the 0.3 appearing above), where
			 * log(1.2) is roughly 0.2.
			 */
			...
			/*
			 * error propagation: log(x+errx) = log(x)+errx/x, so the absolute error in the
			 * result equals the relative error in the input, xUlpDbl/xDbl .
			 */
	...
	public static BigDecimal log(int n, final MathContext mc) {
		...
		if (n <= 0)
			...
			/*
			 * summation of a series roughly proportional to (7/500)^k. Estimate count of
			 * terms to estimate the precision (drop the favorable additional 1/k here):
			 * 0.013^k <= 10^(-precision), so k*log10(0.013) <= -precision so k>=
			 * precision/1.87.
			 */
			...
			/*
			 * log3 is roughly 1, so absolute and relative error are the same. The result
			 * will be divided by 12, so a conservative error is the one already found in mc
			 */
			...
			/*
			 * summation of a series roughly proportional to (7/160)^k. Estimate count of
			 * terms to estimate the precision (drop the favorable additional 1/k here):
			 * 0.046^k <= 10^(-precision), so k*log10(0.046) <= -precision so k>=
			 * precision/1.33.
			 */
			...
			/*
			 * log5 is roughly 1.6, so absolute and relative error are the same. The result
			 * will be divided by 6, so a conservative error is the one already found in mc
			 */
			...
			/*
			 * summation of a series roughly proportional to (1/8)^k. Estimate count of
			 * terms to estimate the precision (drop the favorable additional 1/k here):
			 * 0.125^k <= 10^(-precision), so k*log10(0.125) <= -precision so k>=
			 * precision/0.903.
			 */
			...
			/*
			 * At this point one could either forward to the log(BigDecimal) signature
			 * (implemented) or decompose n into Ifactors and use an implemenation of all
			 * the prime bases. Estimate of the result; convert the mc argument to an
			 * absolute error eps log(n+errn) = log(n)+errn/n = log(n)+eps
			 */
			double res = Math.log((double) n);
			double eps = prec2err(res, mc.getPrecision());
			/*
			 * errn = eps*n, convert absolute error in result to requirement on absolute
			 * error in input
			 */
			eps *= n;
			/*
			...
			/*
			 * Padd n with a number of zeros to trigger the required accuracy in the
	...
	public static BigDecimal log(final Rational r, final MathContext mc) {
		...
		if (r.compareTo(Rational.ZERO) <= 0)
			...
			/*
			 * log(r+epsr) = log(r)+epsr/r. Convert the precision to an absolute error in
			 * the result. eps contains the required absolute error of the result, epsr/r.
			 */
			double eps = prec2err(Math.log(r.doubleValue()), mc.getPrecision());

			/*
			 * Convert this further into a requirement of the relative precision in r, given
			 * that epsr/r is also the relative precision of r. Add one safety digit.
			 */
	...
	public static BigDecimal powRound(final BigDecimal x, final int n) {
		...
		if (n == 1)
			 ...
			 * The estimation is slightly optimistic due to the integer rounding of the
			 * logarithm. Since the standard BigDecimal.pow can only handle positive n, we
			 * split the algorithm.
			 */
	...
	public static BigDecimal powRound(final BigDecimal x, final BigInteger n) {
		/**
		 * For now, the implementation forwards to the cases where n is in the range of
		 * the standard integers. This might, however, be implemented to decompose
		 * larger powers into cascaded calls to smaller ones.
		 */
	 ...
	 * 
	 * @param x The base. Generally enforced to be positive, with the exception of
	 *          integer exponents where the sign is carried over according to the
	...
	public static BigDecimal powRound(final BigDecimal x, final Rational q) {
		...
		if (q.compareTo(BigInteger.ONE) == 0)
			...
			/*
			 * We are sure that the denominator is positive here, because normalize() has
			 * been called during constrution etc.
			 */
			return powRound(x, q.a);
		}
		/*
		 * Refuse to operate on the general negative basis. The integer q have already
		 * been handled above.
		 */
		else if (x.compareTo(BigDecimal.ZERO) < 0)
			throw new ArithmeticException("Cannot power negative " + x.toString());
			...
			if (q.isIntegerFrac()) {
				/*
				 * Newton method with first estimate in double precision. The disadvantage of
				 * this first line here is that the result must fit in the standard range of
				 * double precision numbers exponents.
				 */
				double estim = Math.pow(x.doubleValue(), q.doubleValue());
				BigDecimal res = new BigDecimal(estim);

				/*
				 * The error in x^q is q*x^(q-1)*Delta(x). The relative error is q*Delta(x)/x, q
				 * times the relative error of x.
				 */
				BigDecimal reserr = new BigDecimal(
						0.5 * q.abs().doubleValue() * x.ulp().divide(x.abs(), MathContext.DECIMAL64).doubleValue());

				/*
				 * The main point in branching the cases above is that this conversion will
				 * succeed for numerator and denominator of q.
				 */
				int qa = q.a.intValue();
				int qb = q.b.intValue();
				...
				for (;;) {
					/*
					 * numerator and denominator of the Newton term. The major disadvantage of this
					 * implementation is that the updates of the powers of the new estimate are done
					 * in full precision calling BigDecimal.pow(), which becomes slow if the
					 * denominator of q is large.
					 */
					...
					/*
					 * reached final precision if the relative error fell below reserr, |eps/res| <
					 * reserr
					 */
				...
				/*
				 * The error in x^q is q*x^(q-1)*Delta(x) + Delta(q)*x^q*log(x). The relative
				 * error is q/x*Delta(x) + Delta(q)*log(x). Convert q to a floating point number
				 * such that its relative error becomes negligible: Delta(q)/q <<
				 * Delta(x)/x/log(x) .
				 */
				int precq = 3 + err2prec(
						(x.ulp().divide(x, MathContext.DECIMAL64)).doubleValue() / Math.log(x.doubleValue()));
				MathContext mc = new MathContext(precq);

				/*
				 * Perform the actual calculation as exponentiation of two floating point
				 * numbers.
				 */
	...
	public static BigDecimal sin(final BigDecimal x) {
		if (x.compareTo(BigDecimal.ZERO) < 0)
			...
			if (res.compareTo(p) > 0) {
				...
				/*
				 * for the range 0<=x<Pi/2 one could use sin(2x)=2sin(x)cos(x) to split this
				 * further. Here, use the sine up to pi/4 and the cosine higher up.
				 */
				if (res.multiply(new BigDecimal("4")).compareTo(p) > 0) {
					...
					/*
					 * The error in the result is set by the error in x itself. We need at most k
					 * terms to squeeze x^(2k+1)/(2k+1)! below this value. x^(2k+1) < x.ulp;
					 * (2k+1)*log10(x) < -x.precision; 2k*log10(x)< -x.precision; 2k*(-log10(x)) >
	...
	public static BigDecimal cos(final BigDecimal x) {
		if (x.compareTo(BigDecimal.ZERO) < 0)
			...
			if (res.compareTo(p) > 0) {
				...
				/*
				 * for the range 0<=x<Pi/2 one could use cos(2x)= 1-2*sin^2(x) to split this
				 * further, or use the cos up to pi/4 and the sine higher up. throw new
				 * ProviderException("Not implemented: cosine ");
				 */
				if (res.multiply(new BigDecimal("4")).compareTo(p) > 0) {
					/*
					...
					/*
					 * The absolute error in the result is the error in x^2/2 which is x times the
					 * error in x.
					 */
					double xUlpDbl = 0.5 * res.ulp().doubleValue() * res.doubleValue();

					/*
					 * The error in the result is set by the error in x^2/2 itself, xUlpDbl. We need
					 * at most k terms to push x^(2k+1)/(2k+1)! below this value. x^(2k) < xUlpDbl;
	...
	public static BigDecimal atan(final BigDecimal x) {
		if (x.compareTo(BigDecimal.ZERO) < 0) {
			...
			/*
			 * Abramowitz-Stegun 4.4.34 convergence acceleration 2*arctan(x) =
			 * arctan(2x/(1-x^2)) = arctan(y). x=(sqrt(1+y^2)-1)/y This maps 0<=y<=3 to
			 * 0<=x<=0.73 roughly. Temporarily with 2 protectionist digits.
			 */
			...
			/*
			 * start with the term pi/2; gather its precision relative to the expected
			 * result
			 */
	...
	public static BigDecimal cosh(final BigDecimal x) {
		if (x.compareTo(BigDecimal.ZERO) < 0)
			...
			if (x.doubleValue() > 1.5) {
				...
				/*
				 * The error in the result is set by the error in x^2/2 itself, xUlpDbl. We need
				 * at most k terms to push x^(2k)/(2k)! below this value. x^(2k) < xUlpDbl;
				 * (2k)*log(x) < log(xUlpDbl);
				 */
				int k = (int) (Math.log(xUlpDbl) / Math.log(x.doubleValue())) / 2;

				/*
				 * The individual terms are all smaller than 1, so an estimate of 1.0 for the
				 * absolute value will give a safe relative error estimate for the indivdual
				 * terms
				 */
				MathContext mcTay = new MathContext(err2prec(1., xUlpDbl / k));
				for (int i = 1;; i++) {
	...
	public static BigDecimal sinh(final BigDecimal x) {
		if (x.compareTo(BigDecimal.ZERO) < 0)
			...
			if (x.doubleValue() > 2.4) {
				...
				/*
				 * The error in the result is set by the error in x itself. The first derivative
				 * of sinh(x) is cosh(x), so the absolute error in the result is cosh(x)*errx,
				 * and the relative error is coth(x)*errx = errx/tanh(x)
				 */
	...
	public static BigDecimal Gamma(final BigDecimal x) {
		/*
		 * reduce to interval near 1.0 with the functional relation, Abramowitz-Stegun
		 * 6.1.33
		 */
		if (x.compareTo(BigDecimal.ZERO) < 0)
			...
			/*
			 * measure of the absolute error is the relative error in the first, logarithmic
			 * term
			 */
			...
			if (x.compareTo(BigDecimal.ONE) != 0) {
				...
				for (int n = 2;; n++) {
					/*
					 * multiplying z^n/n by zeta(n-1) means that the two relative errors add. so the
					 * requirement in the relative error of zeta(n)-1 is that this is somewhat
					 * smaller than the relative error in z^n/n (the absolute error of thelatter is
					 * the absolute error in z)
					 */
					...
					/*
					 * At larger n, zeta(n)-1 is roughly 1/2^n. The product is c/2^n. The relative
					 * error in c is c.ulp/2/c . The error in the product should be small versus
					 * eps/10. Error from 1/2^n is c*err(sigma-1). We need a relative error of
					 * zeta-1 of the order of c.ulp/50/c. This is an absolute error in zeta-1 of
					 * c.ulp/50/c/2^n, and also the absolute error in zeta, because zeta is of the
					 * order of 1.
					 */
			...
			/*
			 * The relative error in the result is the absolute error in the input variable
			 * times the digamma (psi) value at that point.
			 */
	...
	public static BigDecimal Gamma(final Rational q, final MathContext mc) {
		if (q.isBigInteger()) {
			...
			/*
			 * The relative error of the result is psi(x)*Delta(x). Tune Delta(x) such that
			 * this is equivalent to mc: Delta(x) = precision/psi(x).
			 */
	...
	public static BigDecimal mod2pi(BigDecimal x) {
		/*
		 * write x= 2*pi*k+r with the precision in r defined by the precision of x and
		 * not compromised by the precision of 2*pi, so the ulp of 2*pi*k should match
		 * the ulp of x. First get a guess of k to figure out how many digits of 2*pi
		 * are needed.
		 */
		int k = (int) (0.5 * x.doubleValue() / Math.PI);

		/*
		 * want to have err(2*pi*k)< err(x)=0.5*x.ulp, so err(pi) = err(x)/(4k) with two
		 * safety digits
		 */
		double err2pi;
		if (k != 0)
		...
		/*
		 * Delegate the actual operation to the BigDecimal class, which may return a
		 * negative value of x was negative .
		 */
	...
	public static BigDecimal modpi(BigDecimal x) {
		 ...
		 * compromised by the precision of pi, so the ulp of pi*k should match the ulp
		 * of x. First get a guess of k to figure out how many digits of pi are needed.
		 */
		int k = (int) (x.doubleValue() / Math.PI);

		/*
		 * want to have err(pi*k)< err(x)=x.ulp/2, so err(pi) = err(x)/(2k) with two
		 * safety digits
		 */
		double errpi;
		if (k != 0)
	...
	public static BigDecimal zeta(final int n, final MathContext mc) {
		...
		if (n % 2 == 0) {
			 ...
			 * error in pi times pi^(n-1). Relative error is n*error(pi)/pi, requested by
			 * mc. Need one more digit in pi if n=10, two digits if n=100 etc, and add one
			 * extra digit.
			 */
			...
			/*
			 * Broadhurst BBP <a
			 * href="http://arxiv.org/abs/math/9803067">arXiv:math/9803067</a> Error
			 * propagation: S31 is roughly 0.087, S33 roughly 0.131
			 */
			 ...
			 * Broadhurst BBP <a
			 * href=http://arxiv.org/abs/math/9803067">arXiv:math/9803067</a> Error
			 * propagation: S51 is roughly -11.15, S53 roughly 22.165, S55 is roughly 0.031
			 * 9*2048*S51/6265 = -3.28. 7*2038*S53/61651= 5.07. 738*2048*S55/61651= 0.747.
			 * The result is of the order 1.03, so we add 2 digits to S51 and S52 and one
			 * digit to S55.
			 */
			...
			/*
			 * The first term, including the facor (2pi)^n, is essentially most of the
			 * result, near one. The second term below is roughly in the range 0.003 to
			 * 0.009. So the precision here is matching the precisionn requested by mc, and
			 * the precision requested for 2*pi is in absolute terms adjusted.
			 */
			...
			if (n % 4 == 3) {
				/*
				 * since the argument n is at least 7 here, the drop of the terms is at rather
				 * constant pace at least 10^-3, for example 0.0018, 0.2e-7, 0.29e-11, 0.74e-15
				 * etc for npr=1,2,3.... We want 2 times these terms fall below eps/10.
				 */
				...
				for (int npr = 2; npr <= kmax; npr++) {
					/*
					 * the error estimate above for npr=1 is the worst case of the absolute error
					 * created by an error in 2pi. So we can safely re-use the exp2p value computed
					 * above without reassessment of its error.
					 */
				...
				/*
				 * since the argument n is at least 9 here, the drop of the terms is at rather
				 * constant pace at least 10^-3, for example 0.0096, 0.5e-7, 0.3e-11, 0.6e-15
				 * etc. We want these terms fall below eps/10.
				 */
				int kmax = (1 + mc.getPrecision()) / 3;
				eps /= kmax;
				/*
				 * need an error of eps for 2/(exp(2pi)-1)*(1+4*Pi/8/(1-exp(-2pi)) = 0.0096 at
				 * k=7 or = 0.00766 at k=13 for example. The absolute error is 0.017*err(pi) at
				 * k=9, 0.013*err(pi) at k=13, 0.012 at k=17
				 */
	...
	public static double zeta1(final int n) {
		...
		if (n < zmin1.length)
			...
			/*
			 * Result is roughly 2^(-n), desired accuracy 18 digits. If zeta(n) is computed,
			 * the equivalent accuracy in relative units is higher, because zeta is around
			 * 1.
			 */
	 ...
	 * 
	 * @param x The main argument.
	 * @return psi(x). The error is sometimes up to 10 ulp, where AS 6.3.15 suffers
	 *         from cancellation of digits and psi=0
	 * @since 2009-08-26
	 * @author Richard J. Mathar
	 */
	public static double psi(final double x) {
		...
		if (x > 2.0) {
			...
			/* 0.422... = 1 -gamma */
			return resul + 0.422784335098467139393487909917597568 + 0.5 / xmin1 - 1. / (1 - xmin1 * xmin1)
					- Math.PI / (2. * Math.tan(Math.PI * xmin1));
		}
	} /* psi */
	...
	static protected BigDecimal broadhurstBBP(final int n, final int p, final int a[], MathContext mc) {
		...
		/*
		 * Convert the relative precision and estimate of the result into an absolute
		 * precision.
		 */
		double eps = prec2err(x, mc.getPrecision());

		/*
		 * Divide this through the number of terms in the sum to account for error
		 * accumulation The divisor 2^(p(k+1)/2) means that on the average each 8th term
		 * in k has shrunk by relative to the 8th predecessor by 1/2^(4p). 1/2^(4pc) =
		 * 10^(-precision) with c the 8term cycles yields c=log_2( 10^precision)/4p =
		 * 3.3*precision/4p with k=8c
		 */
	...
	public static Rational toRational(BigDecimal x) {
		/*
		 * represent the floating point number by the exact rational variant of the
		 * current truncated representation
		 */
		int s = x.scale();
		if (s > 0)
	 ...
	 * @return A list of the form [a0,a1,a2,a3,...] where The decomposition is |x| =
	 *         a0+1/(a1+1/(a2+1/(a3+...))).
	 * @since 2012-03-09
	 * @author Richard J. Mathar
	 */
	...
	public static BigDecimal divideRound(final BigDecimal x, final BigDecimal y) {
		...
		/*
		 * If x and y are precise integer values that may have common factors, the
		 * method above will truncate trailing zeros, which may result in a smaller
		 * apparent accuracy than starte... add missing trailing zeros now.
		 */
	...
	/**
	 * Boost the precision by appending decimal zeros to the value. This returns a
	 * value which appears to have a higher precision than the input.
	 * 
	 * @param x  The input value
	 ...
	 * @return The number of valid digits in x. Derived from the representation x+-
	 *         xerr, as if the error was represented in a "half width" (half of the
	 *         error bar) form. The value is rounded down, and on the pessimistic
	...
	public static int err2prec(double x, double xerr) {
		/*
		 * Example: an error of xerr=+-0.5 at x=100 represents 100+-0.5 with a precision
		 * = 3 (digits).
		 */
	...
	public static int err2prec(double xerr) {
		/*
		 * Example: an error of xerr=+-0.5 a precision of 1 (digit), an error of +-0.05
	...
	/**
	 * Convert a precision (relative error) to an absolute error. The is the inverse
	 * functionality of err2prec().
	 * 
	 * @param x    The value of the variable The value returned depends only on the
	 *             absolute value, not on the sign.
	 * @param prec The number of valid digits of the variable.
	 * @return the absolute error in x. Derived from the an accuracy of one half of
	 *         the ulp.
	 * @since 2009-08-09
	 * @author Richard J. Mathar
	 */
