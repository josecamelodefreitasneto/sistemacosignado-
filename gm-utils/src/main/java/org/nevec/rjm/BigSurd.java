...
public class BigSurd implements Cloneable, Comparable<BigSurd> {
	...
	/**
	 * The number underneath the square root, always non-negative. The mathematical
	 * object has the value pref*sqrt(disc).
	 */
	Rational disc;
	...
	/**
	 * ctor given the prefactor and the basis of the root. This creates an object of
	 * value a*sqrt(b).
	 * 
	...
	/**
	 * ctor given the numerator and denominator of the root. This creates an object
	 * of value sqrt(a/b).
	 * 
	...
	public BigSurd clone() {
		...
		/*
		 * the main intent here is to bypass any attempt to reduce the discriminant by
		 * figuring out the square-free part in normalize(), which has already done in
		 * the current copy of the number.
		 */
	...
	public BigSurd negate() {
		/*
		 * This is trying to be quick, avoiding normalize(), by toggling the sign in a
		 * clone()
		 */
		BigSurd n = clone();
	...
	public int compareTo(final BigSurd val) {
		/*
		 * Since we keep the discriminant positive, the rough estimate comes from
		 * comparing the signs of the prefactors.
		 */
		...
		/*
		 * Work out the cases of equal sign. Compare absolute values by comparison of
		 * the squares which is forwarded to the comparison of the Rational class.
		 */
		...
		if (c == 0)
			return 0;
		/*
		 * If both values have negative sign, the one with the smaller square is the
		 * larger number.
		 */
	...
	public double doubleValue() {
		/*
		 * First compute the square to prevent overflows if the two pieces of the
		 * prefactor and the discriminant are of very different magnitude.
		 */
	...
	/**
	 * The sign: 1 if the number is larger than 0, 0 if it equals zero, -1 if it is
	 * less than zero
	 * 
	...
	public int signum() {
		 ...
		 * prefactor. This works because a zero discriminant is always copied over to
		 * the prefactor, not hidden.
		 */
		return pref.signum();
	} /* BigSurd.signum */
	...
	protected void normalizeG() {
		/*
		 * Is there a common factor between the numerator of the prefactor and the
		 * denominator of the discriminant ?
		 */
		BigInteger d = pref.numer().abs().gcd(disc.denom());
		if (d.compareTo(BigInteger.ONE) > 0) {
			pref = pref.divide(d);
			/*
			 * instead of multiplying with the square of d, using two steps offers a change
			 * to recognize the common factor..
			 */
			disc = disc.multiply(d);
			disc = disc.multiply(d);
		}
		/*
		 * Is there a common factor between the denominator of the prefactor and the
		 * numerator of the discriminant ?
		 */
		d = pref.denom().gcd(disc.numer());
		if (d.compareTo(BigInteger.ONE) > 0) {
			pref = pref.multiply(d);
			/*
			 * instead of dividing through the square of d, using two steps offers a change
			 * to recognize the common factor..
			 */
			disc = disc.divide(d);
			disc = disc.divide(d);
	...
	public BigDecimal BigDecimalValue(MathContext mc) {
		/*
		 * the relative error of the result equals the relative error of the prefactor
		 * plus half of the relative error of the discriminant. So adding 3 digits
