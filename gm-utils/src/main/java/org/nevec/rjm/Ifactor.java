...
public class Ifactor implements Cloneable, Comparable<Ifactor> {
	 ...
	 * represented by an empty vector, the value 1 by a vector of length 1 with a
	 * single power of 0.
	 */
	 ...
	 *             exponent of 2, pows[1] the exponent of 3, pows[2] the exponent of
	 *             5 etc. Note that this list does not include the primes, but
...
//		Vector<Integer> p = (Vector<Integer>) primeexp.clone();
	 ...
	 * Vector.contains() calls that use the value, not the reference for comparison.
	 * 
	...
	public Ifactor multiply(final Ifactor oth) {
		/*
		 * This might be done similar to the lcm() implementation by adding the powers
		 * of the components and calling the constructor with the list of exponents.
		 * This here is the simplest implementation, but slow because it calls another
		 * prime factorization of the product: return( new Ifactor(n.multiply(oth.n)));
		 */
	 ...
	 * @return the lowest common multiple of both numbers. Returns zero if any of
	 *         both arguments is zero.
	 * @author Richard J. Mathar
	 */
	...
	protected Ifactor multGcdLcm(final Ifactor oth, int type) {
		...
		/*
		 * skip the case where 0*something =0, falling thru to the empty representation
		 * for 0
		 */
		if (primeexp.size() != 0 && oth.primeexp.size() != 0) {
			/*
			 * Cases of 1 times something return something. Cases of lcm(1, something)
			 * return something. Cases of gcd(1, something) return 1.
			 */
			if (primeexp.firstElement().intValue() == 1 && type == 0)
				...
				while (idxOth < oth.primeexp.size() || idxThis < primeexp.size()) {
					if (idxOth >= oth.primeexp.size()) {
						/*
						 * exhausted the list in oth.primeexp; copy over the remaining 'this' if
						 * multiplying or lcm, discard if gcd.
						 */
						if (type == 0 || type == 2) {
	...
	public Ifactor divide(final Ifactor oth) {
		/*
		 * todo: it'd probably be faster to cancel the gcd(this,oth) first in the prime
		 * power representation, which would avoid a more strenous factorization of the
		 * integer ratio
		 */
	...
	public Rational root(final int r) throws ArithmeticException {
		if (r == 0)
			...
			for (int i = 0; i < primeexp.size(); i += 2) {
				/*
				 * all exponents must be multiples of r to succeed (that is, to stay in the
				 * range of rational results).
				 */
				int ex = primeexp.elementAt(i + 1).intValue();
				if (ex % r != 0)
			...
			/*
			 * convert result to a Rational; unfortunately this will loose the prime
			 * factorization
			 */
	...
	public Vector<BigInteger> divisors() {
		/*
		 * Recursive approach: the divisors of p1^e1*p2^e2*..*py^ey*pz^ez are the
		 * divisors that don't contain the factor pz, and the divisors that contain any
		 * power of pz between 1 and up to ez multiplied by 1 or by a product that
		...
		if (n.compareTo(BigInteger.ONE) > 0) {
			...
			/*
			 * the output contains all products of the form partd[]*pz^ez, ez>0, and with
			 * the exception of the 1, all these are appended.
			 */
	...
	public Ifactor sigma(int k) {
		/*
		 * the question is whether keeping a factorization is worth the effort or
		 * whether one should simply multiply these to return a BigInteger...
		 */
		if (n.compareTo(BigInteger.ONE) == 0)
			return ONE;
			...
			for (int i = 0; i < primeexp.size(); i += 2) {
				...
				if (k == 0)
					...
					/*
					 * This division is of course exact, no remainder The costly prime factorization
					 * is hidden here.
					 */
	 ...
	 * 
	 * @return the new integer obtained by removing the highest prime power. If this
	 *         here represents 0 or 1, it is returned without change.
	 * @since 2006-08-20
	 * @author Richard J. Mathar
	 */
	public Ifactor dropPrime() {
		...
		/*
		 * The cases n>1 Start empty. Copy all but the last factor over to the result
		 * the vector with the new prime factor powers, which contain the old prime
		 * factor powers up to but not including the last one.
		 */
		Ifactor pows = new Ifactor(0);
...
//		boolean resul = true;
	...
	/**
	 * The Moebius function. 1 if n=1, else, if k is the number of distinct prime
	 * factors, return (-1)^k, else, if k has repeated prime factors, return 0.
	 * 
