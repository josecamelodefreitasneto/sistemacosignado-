...
* @since 2014-06-19
*/
public class Partit implements Iterator<int[]> {
	/**
	...
	/**
	 * The current (most recent) part emitted. This is equivalent to the minimum of
	 * any partitions emitted in the future, because we are emitting them in
	 * increasing order.
	 */
	int curPart;
	...
	/**
	 * A flag which is true if compositions of n are constructed. False if
	 * partitions of n are constructed.
	 * 
	 * @since 2015-03-18
	 */
	boolean composit;
	...
	public Partit(int n, boolean doComposit) {
		/*
		 * forward this to the constructor which puts a lower bound of 1 and a maximum
		 * cap of n to the parts to be admitted.
		 */
	...
	/**
	 * Constructor defining the integer to be decomposed and a lower bound on all
	 * parts.
	 * 
	 * @param n          The integer to be decomposed into partitions.
	 ...
	 * @param doComposit Set to true if compositions instead of partitions are to be
	 *                   generated.
	 * @author Richard J. Mathar
	 */
	public Partit(int n, int minp, int maxp, boolean doComposit) {
		...
		if (curPart >= n || curPart > maxpart) {
			/*
			 * The requirements of n<=maxp cannot be met and the requirement of minp<=n can
			 * be met at most once (by setting the next part to be equal to n=curPart).
			 */
			trail = null;
		} else {
			/*
			 * The requirements of minp<=n<=maxp cann be met. If we book 'curPart' as the
			 * first part in the upcomming partitions, the rest of the partitions need to
			 * partition n minus curPart and have to take 'curPart' as their new minimum.
			 */
	 ...
	 * 
	 * @param parts The list of positive integers providing the partition. The
	 *              integers do not need to be sorted in any particular way.
	 ...
	 * @return The non-negative count of n in the parts. This is somewhere between 0
	 *         and the number of elements in parts[] (inclusive).
	 * @since 2014-07-14
	 * @author Richard J. Mathar
	 */
	...
	/**
	 * Compute the frequency of part larger than or equal to some minimum in a
	 * partition.
	 * 
	 * @param parts The list of integers with the partition.
	 ...
	 * Iterator interface.
	 * 
	 * @return true If some of the partitions have not yet been constructed with
	 *         next(). This is equivalent to saying that the next call to next()
	...
	public boolean hasNext() {
		/*
		 * this is a sort of dry run through next(). We can sequeeze at least one other
		 * part if the remaining sum (which is n) is >= curPart.
		 */
	 ...
	 * 
	 * @return An integer vector of the parts. null if all partitions have already
	 *         been emitted.
	 * @author Richard J. Mathar
	 */
	public int[] next() {
	...
	/**
	 * Print the partitions of an integer. The integer is specified by command line
	 * argument. A maximum of each part in the partition may be added as another
	 * argument Usage: java -classpath . de.mpg.mpia.rjm.Partit n [m]
	 * 
	 * @param args
	 * @since 2014-06-19
	 * @author Richard J. Mathar
	...
	public static void main(String[] args) {
		if (args.length > 0) {
			int n = UInteger.toInt(args[0]);
			int m = n;
			if (args.length > 1) {
				m = UInteger.toInt(args[1]);
			}
